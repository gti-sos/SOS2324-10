<script>
	...
	function createGraphI(data) {
		const graphData = data.reduce(
			(acc, current) => {
				const locationIndex = acc.locations.indexOf(current.location);
				if (locationIndex === -1) {
					acc.locations.push(current.location);
					acc.counts.push(1);
				} else {
					acc.counts[locationIndex]++;
				}
				return acc;
			},
			{ locations: [], counts: [] }
		...
		const trace = {
			labels: graphData.locations,
			values: graphData.counts,
			type: 'pie',
		...
		const layout = {
			title: 'Conciertos por Ubicación',
