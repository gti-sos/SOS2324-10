<script>
	...
	if (dev) API_MRF = 'http://localhost:8080' + API_MRF;
	...
	async function getGDP() {
		await getGDPTotal();
		try {
			//Calcula el inicio de la paginacion
			let offset = (currentPage - 1) * pageSize;

			//Construye llamada a la API con el limite y la paginacion
	...
	// Filtrado por atributos (Se puede filtrar por cualquier atributo, y rango de años (desde, hasta))
	...
	//FUncion para borrar el filtro facilmente con un objeto nuevo desde 0
	...
	async function createGDP() {
		try {
			...
			if (response.ok) {
                //Desaparece el formulario de creacion
				showForm = false;
				getGDP();
    ...
    //Funcion eliminar dato concreto
...
{#if gdp && gdp.length > 0}
				...
				{#each gdp as dato}
					<tr>
						<td>
							<!-- Botón de detalles -->
							<a
								href="gdp-growth-rates/{dato.geo}/{dato.time_period}"
				...
				<form on:submit|preventDefault={getFilter}>
					<label>
						...
						<input
							...
							id="frequency"
							placeholder="frequency"
							bind:value={filteredGdp.frequency}
							style="margin-bottom: 10px;"
						/>
					</label>
					<label>
					...
					<label>
						...
						<input
							...
							id="na_item"
							placeholder="na_item"
							bind:value={filteredGdp.na_item}
							style="margin-bottom: 10px;"
						/>
					</label>
					<label
					...
					<label>
						...
						<input
							...
							id="time_period"
							placeholder="time_period"
							bind:value={filteredGdp.time_period}
							style="margin-bottom: 10px;"
						/>
					</label>
					<label>
						...
						<input
							...
							id="obs_value"
							placeholder="frequency"
							bind:value={filteredGdp.obs_value}
							style="margin-bottom: 10px;"
						/>
					</label>
					<label>
						...
						<input
							...
							id="growth_rate_2030"
							placeholder="growth_rate_2030"
							bind:value={filteredGdp.growth_rate_2030}
							style="margin-bottom: 10px;"
						/>
					</label>
					<label
						...
						<input
							...
							id="growth_rate_2040"
							placeholder="growth_rate_2040"
							bind:value={filteredGdp.growth_rate_2040}
							style="margin-bottom: 10px;"
						/>
					</label>
					<label
    ...
    <!-- Mostramos mensajes de error o de exito-->
...
{:else}
    ...
    <!--- Botón previo de carga de datos iniciales-->
