...
<script>
    ...
    async function getVehicles() {
        try {
            let response = await fetch(API_TLR_ALL, {
                method: 'GET'
            });
...
{#if datos && datos.length > 0}
    ...
    <div style="text-align: right; margin-top: 20px;">
        <button on:click={() => { showForm = true; }}>Crear Entrada</button>
    </div>
        ...
        <div class="modal">
            <div class="modal-content">
                ...
                <form on:submit|preventDefault={postVehicle}>
                    ...
                    <label>
                        Flights Passangers:
                        <input type="number" bind:value={newDato.flights_passangers} style="margin-bottom: 10px;">
                    </label>
                    <label>
                        Cars Deaths:
